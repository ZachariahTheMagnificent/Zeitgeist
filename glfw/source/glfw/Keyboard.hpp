...
namespace opengl
{
	...
	{
	public:
		explicit Key ( const KEYS key, const KEY_STATE state, const std::bitset<4> modifiers, const int scancode ) noexcept;

		KEYS GetKey ( ) const noexcept;
		KEY_STATE GetState ( ) const noexcept;
		int GetScanCode ( ) const noexcept;

		template<MODIFIERS modifier>
		bool GetModifier ( ) const noexcept
		{
			switch ( modifier )
			{
